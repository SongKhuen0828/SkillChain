...
vec3 getLineColor(float t, vec3 baseColor) {
  ...
  vec3 gradientColor;
  if (lineGradientCount == 1) { gradientColor = lineGradient[0]; } else {
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const getLineCount = (t: any) => {
      if (typeof lineCount === 'number') return lineCount
      const idx = enabledWaves.indexOf(t)
      return idx >= 0 ? lineCount[idx] ?? 6 : 0
    }
    const getLineDist = (t: any) => {
      if (typeof lineDistance === 'number') return lineDistance
      const idx = enabledWaves.indexOf(t)
      return idx >= 0 ? lineDistance[idx] ?? 0.1 : 0.1
    }
